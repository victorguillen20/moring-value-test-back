...
public class CombateService implements CombateUseCase {
    ...
    @Override
    public Combate iniciar(IniciarCombateCommand cmd) {
        if (cmd.getEspecie1Id() == null || cmd.getEspecie2Id() == null) {
            throw new CombateInvalidoException("Los IDs de las especies son obligatorios");
        }

        Especie especie1 = especieRepository.findById(cmd.getEspecie1Id())
                .orElseThrow(() -> new EspecieNoEncontradaException(
                        "La especie con id " + cmd.getEspecie1Id() + " no existe"));

        Especie especie2 = especieRepository.findById(cmd.getEspecie2Id())
                .orElseThrow(() -> new EspecieNoEncontradaException(
                        "La especie con id " + cmd.getEspecie2Id() + " no existe"));
        ...
        Combate combate = new Combate(especie1, especie2, ganador,
                nivelEf1, nivelEf2, mod1, mod2, LocalDateTime.now(clock));
    ...
    public Combate iniciarAleatorio() {
        ...
        IniciarCombateCommand cmd = new IniciarCombateCommand(especie1.getId(), especie2.getId(), false);
        return this.iniciar(cmd);
    }
